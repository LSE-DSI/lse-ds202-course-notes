...
Last week we learnt how to do the Classification Methods in R. As data scientists, we need to know how to examine the results we generated and also to justify our results.  So, we will focus on and practice the validation methods in the R language in this week's lab. 
...
<details>
<summary> Step 2: Leave-One-Out Cross-Validation (LOOCV) (15mins) </summary>

## Step 2: Leave-One-Out Cross-Validation (LOOCV)

Hereby, We will compute the LOOCV values by using the function `glm` rather than `lm`.

### Step 2.1: Generate the cross-validation results with the generalized linear model.
...
### Step 2.2: Repeat the former step and fit increasingly complex polynomial models
...
I would also encourage you to basically visualise the results using `ggplot`. We can clearly see a sharp drop in the estimated test $\operatorname{MSE}$ between the linear and quadratic fits, from $24.27$ to $19.27$, but then no clear improvement from using higher-order polynomials. 
...
<details><summary> Step 5: 🎯 Practical Exercises (35mins)</summary>
