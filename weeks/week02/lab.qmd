...
4. Create a heatmap with `ggplot2` package:
    ...
    > library(ggplot2)
    ```
    To create a dummy dataframe, we can use country names, a time period and random GDP for each country.
    ...
    Now to create random GPD for each country and each year, and add these values into our dataframe as GDP column::
...
3.  `AUTO` dataset is ready to be used in the analyse. You can explore the dataset by using:
    ...
    the `View()` function can be used to view it in a spreadsheet-like window. The `head()` function can also be used to view the first few rows of the data
