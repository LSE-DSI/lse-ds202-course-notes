...
6. Use the `lm()` function to perform a multiple linear regression，and use the `summary()` function to print the results:
    ...
    i. Yes, there is a relatioship between the predictors and the response by testing the null hypothesis of whether all the regression coefficients are zero. The F-statistic is far from 1 (with a small p-value), indicating evidence against the null hypothesis.
    ...
    iii. The regression coefficient for year, 0.75, suggests that for a given amount of the other predicators, every year mpg increases additional 0.75 unit. In other words, cars become more fuel efficient every year by almost 1 mpg/year.
...
7. Use the `plot()` function to produce diagnostic plots:
    ...
    ```
     ...
     We could also find from the leverage plot, point 14 appears to have high leverage, although not a high magnitude residual. Besides, the quadratic trend of the residuals could be a problem. Maybe linear regression is not the best fit for this prediction. Then we plot studentized residuals to identify outliers:
