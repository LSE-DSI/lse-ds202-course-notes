...
4. Use the function `summary()` to understand detailed information about the model lm.fit we just fitted, like p-values and standard errors for the coefficients, as well as the R<sup>2</sup> statistic and F-statistic:
    ...
    Because `lm.fit` is a simple linear regression model, there are only two coefficients: &beta;<sub>0</sub> and &beta;<sub>1</sub>. The Coefficients in the output shows the estimated value, standard error, t-test result and p-value for t-test of &beta;<sub>0</sub> and &beta;<sub>1</sub>. The goodness-of-fit of the model can be measured by the RSE and R<sup>2</sup> in the output.
...
2. Identify outliers through residual plots:
    ...
    An outlier is a point for which `y<sub>i</sub>` is far from the value predicted by the model. Outliers can arise for a variety of reasons, such as incorrect recording of observation during data collection. Outliers could be identified through residual plots:
