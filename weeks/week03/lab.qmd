...
This week, we will fit simple linear regression model and multiple linear regression model in R, and learn to interpret the R output. We will apply theoretical model to practical cases and deal with some potential problems.
...
1. Install and load the `ISLR2` package which contains a very large collection of data sets and functions.
...
2.  Use the `Boston` data set in the `ISLR2` library. It records `medv` (median house value) for 506 census tracts in Boston. Have a look at first few rows of the Boston data set:
    ...
    We will seek to predict `medv` using 12 predictors such as `rm` (average number of rooms per house), `age` (average age of houses), and `lstat` (percent of households with low socioeconomic status). To find out more about the data set, we can type `?Boston`.
...
4.  Use the function `summary()` to understand detailed information about the model lm.fit we jsut fitted, like p-values and standard errors for the coefficients, as well as the R<sup>2</sup> statistic and F-statistic:
    ...
    Because `lm.fit` is a simple linear regression model, there are only two coefficients: &beta;<sub>0</sub> and &beta;<sub>1</sub>. The Coefficients in the output shows the estimated value, standard error, t-test result and p-value for t-test of &beta;<sub>0</sub> and &beta;<sub>1</sub>. The model fit could be measured by the RSE and R<sup>2</sup> in the output.

    We can also use `lm.fit$coefficients` or `coef(lm.fit)` to check the coeficients of the regression model:
...
6. Predict the value of `medv` for a given value of `lstat` and generate the confidence intervals of the predicted value:
   ...
   For instance, the 95 % confidence interval associated with a lstat value of 10 is (24.47, 25.63). As expected, the confidence is centered around the same point (a predicted value of 25.05 for medv when lstat equals 10).
...
3. Select variables:
    ...
    In these two multiple linear regression models, the results of t-tests and F-test suggest that all predictors are significant for the response variable. However,  it is more often the case that the response is only associated with a subset of the predictors. The task of determining which predictors are associated with the response, in order to fit a single model involving only those predictors, is referred to as variable selection.

    If the number of predictors is very small, we could perform variable selection by trying out a lot of different models, each containing a different subset of the predictors. We can then select the best model out of all of the models that we have considered.

    If the number of predicators is not very small, the function `regsubsets()` could be used to select the best subset of variables. 
...
When we fit a linear regression model to a particular data set, many problems may occur. These problems will lead to inaccurate estimation. In this step, we will identify and overcome some potential problems such as outliers, collinearity and interaction effects. 

We just present a few of the many methods available, but those interested can explore more after class.

1. Handle interaction terms:
    
    In regression, an interaction effect exists when the effect of an independent variable on the response variable changes, depending on the values of one or more other independent variables. When you believe there is interaction effect, it is easy to include interaction terms in a linear model using the `lm()` function.
...
2. Identify outliers through residual plots:
    ...
    An outlier is a point for which `y<sub>i</sub>` is far from the value predicted by the model. Outliers can arise for a variety of reasons, such as incorrect recording of an observation during data collection. Outliers could be identified through residual plots:
    ...
    Four diagnostic plots are automatically produced by applying the `plot()` function directly to the output from `lm()`. Plots on the left column are residual plots, indicating the relatinship between residuals and fitted values.

    In practice, it can be difficult to decide how large a residual needs to be before we consider the point to be an outlier. To address this problem, instead of plotting the residuals, we can plot the studentized residuals, computed by dividing each residual e<sub>i</sub> by its estimated standard studentized residual error. Observations whose studentized residuals are greater than 3 in absolute value are possible outliers. Using the plot() function to plot the studentized residuals:
...
4. Detect collinearity using the correlation matrix:
    ...
    Igonoring the last row and the last column in the matrix which indicate the relatinship with response variable `medv`, an element of this matrix that is large in absolute value indicates a pair of highly correlated variables, and therefore a collinearity problem in the data.
...
So far, we have learnt to fit simple and multipel linear regression medels in R. In this practical case, we will continues with the data set `Auto` studied in last lab. Make sure that the missing values have been removed from the data.

Eight questions are listed below. You are required to try to answer these questions in pair using R commands. We will go over the solutions once everyone has finished these questions.
...
4. Produce a scatterplot matrix which includes all of the variables in the data set.
