...
$R^2$ will always stay the same or increase you add more features to a linear regression, even if those features are "useless".
...
$$
R^2 = \frac{1 - \operatorname{RSS}}{\operatorname{TSS}}
$$
...
$$
\operatorname{RSS}_{(p=1)} = \sum_i^n{e_i^2} = \sum_i^n{\left(y_i - \hat{y}_i'\right)} = \sum_i^n{\left(y_i - \beta_0' - \beta_1' x_{i1}\right)^2}
$$
...
$$
\begin{align}
\operatorname{RSS}_{(p=2)} =& \sum_i^n{\left(y_i - \hat{y}_i''\right)}\\
