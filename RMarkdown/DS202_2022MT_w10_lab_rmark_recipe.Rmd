...
🤝 **WORKING TOGETHER.** In groups or in pairs, list which variables were listed under each role in the `recipe` object above.

> your notes here

## Step 2.2 Change roles
...
🤝 **WORKING TOGETHER.** How would you write a recipe to predict `Today` using all variables of the `SMarket`, except `Direction`?
...
\#TODO: Show example of svm_rbf() with the recipe above. Show that it does not use the variable `Direction`.

## Step 2.3: Unsupervised case
...
```
🗣️ **CLASSROOM DISCUSSIONS**: Does the above make sense to you? Do you see why we don't have an `outcome` role?

🤝 **WORKING TOGETHER.** Now, working in pairs or groups, write a recipe for `class_group` and then change the role of `period_id` to `role="id"`:
