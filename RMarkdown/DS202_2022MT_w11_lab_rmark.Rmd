...
In our final lab, you will learn how to use [quanteda](https://quanteda.io/), an R package for quantitative text analysis. We will do a bit more of data pre-processing, and you will have the chance to apply dimensionality reduction (e.g. PCA) and clustering (e.g. k-means) techniques to text data.
...
:dart: **ACTION POINT:** Before we even open it in R, take some time to look at the data in its raw format. Open the file `PAC_Apologies_Public-Version-2-modified.xlsx` using MS Office.
...
:bulb: *You might have noticed that I am using the explicit name of the functions (`dplyr::mutate` instead of `mutate`, etc.). In the Summative 02, many of you experienced problems running our code because you had other R packages with functions of the same name. So, to avoid any surprises, we will use the full name of functions in several places here. (It's also good as it helps to recognise where the functions come from).*
...
:thinking: How would you do that when we do not have abbreviated country names? And how would you treat the cases of missing values in the `Country (Receiver)` column?

:warning: For now, just believe me: it is possible, the code below deals precisely with that. BUT it is a bit much to digest this early in a lab, so for now let's just run it and observe the output in the `Apology ID` column.

Later, as a :house: **TAKE-HOME ACTIVITY:**, try to understand every single step of the huge pipe below. Ask questions on Slack if you do not get any detail!
...
:dart: **ACTION POINT** Look at the dataframe that represents the document-frequency matrix with either `View(dfm_as_data_frame)` or `head(dfm_as_data_frame)`

:speaking_head: **CLASSROOM DISCUSSIONS**: What do you think all of these columns represent?
...
:speaking_head: **CLASSROOM DISCUSSIONS**: Do you think these most frequent terms are useful in helping us understand the political apologies? Would you remove certain tokens?

*Have you heard of the term 'stop words' before? Ask your class teacher about this! Quanteda has a built-in list of commonly used stop words, view it by running the function `quanteda::stopwords("en")`.*
...
:dart: **ACTION POINT:** Use `kwic` to search for keywords associated with different forms of explicit description of apologies:

``` r
quanteda::kwic(tokens_pac %>% head(n=10), pattern="apolog*")
```

:speaking_head: **CLASSROOM DISCUSSIONS**: What do you think the `*` in the `pattern="apolog*"` parameter does?
...
:dart: **ACTION POINT:** Take a look at `View(df_new)`
...
:house: **TAKE-HOME (OPTIONAL) ACTIVITY:** We used to have 367 rows, now we have 327. How would you change the `pattern` so as to avoid excluding data from the original dataframe? (Note: I do not have a ready solution to this! Feel free to share yours.)
...
:speaking_head: **CLASSROOM DISCUSSIONS**: What do you think of the wordcloud above, compared to the previous one? How would it be different if we had removed unnecessary tokens but kept the original longer description?
...
:speaking_head: **CLASSROOM DISCUSSIONS:** What would you say? How many clusters are in there?
...
```{r}
df %>% 
  mutate(new_timezone=if_else(X5 == 1, 
                              as.POSIXct(X10, origin=, timezone=""), 
                              if_else(X5 == 2, 
                                      as.POSIXct(X10, origin=, tz=""),
                                      as.POSIXct(X10, origin=, tz="" ))   ))
```
...
tstat_key <- textstat_keyness(dfm_pac, target = df$.cluster == 4)
